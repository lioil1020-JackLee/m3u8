...
3. 選擇要下載的集數（支持靈活格式）
4. 選擇輸出資料夾
5. 選擇暫存根目錄（Temp folder，可用 Browse；例如 RAM Disk 路徑）
6. 點擊 "Start" 開始下載
...
若在 GUI 中有填寫 `Temp folder`，會優先使用該路徑作為暫存根目錄。

## 影音不同步改善

本工具在合併 TS 為 MP4 時，預設使用 FFmpeg 的同步修正參數：

- `-fflags +genpts`
- `-af aresample=async=1:first_pts=0`
- `-max_interleave_delta 0`

這會重新整理時間戳並修正音訊時間軸，能顯著降低「聲音先跑/慢半拍」問題。

若你更重視速度，可改用：

```bash
python m3u8.py --no-sync-fix
```
